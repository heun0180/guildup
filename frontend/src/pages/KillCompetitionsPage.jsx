...
export default function KillCompetitionsPage() {
  ...
  useEffect(() => {
    ...
    setAssignments(next);
  }, [detail?.id, detail?.status, detail?.teams.length]);
  ...
  async function createCompetition(event) {
    ...
    try {
      ...
      window.location.href = `/kill-competitions.html?communityId=${encodeURIComponent(communityId)}&competitionId=${created.id}`;
...
function CompetitionList({ items, loading, showCreate, setShowCreate, createCompetition, busy, message }) {
  ...
  return <>
    <div className="page-heading kill-heading"><div><p className="eyebrow">Clan Challenge</p><h1>킬내기</h1>
      <p>클랜원과 팀을 만들고 정해진 시간 동안 킬 경쟁을 진행합니다.</p></div>
...
function CompetitionDetail({ detail, loading, message, busy, now, communityId, mutate, teamCount, setTeamCount,
  assignments, setAssignments, saveTeams }) {
  ...
  const canInterim = detail.status === "IN_PROGRESS" && detail.myParticipantId;
  ...
  return <>
    ...
    {message && <p className="message" role="alert">{message}</p>}
    <section className={`panel kill-state-banner status-${detail.status.toLowerCase()}`}><div><strong>{statusLabel(detail.status)}</strong><p>{statusMessage(detail.status)}</p></div>
      {(detail.status === "IN_PROGRESS" || detail.status === "ENDED") && <div className="kill-countdown"><span>남은 시간</span><strong>{remainingLabel(detail.endsAt, now)}</strong></div>}</section>
    <section className="panel kill-overview"><dl><div><dt>방식</dt><dd>{detail.gameMode}</dd></div><div><dt>시작</dt><dd>{formatDateTime(detail.startedAt)}</dd></div>
      <div><dt>종료</dt><dd>{formatDateTime(detail.endsAt)}</dd></div><div><dt>마지막 정산</dt><dd>{formatDateTime(detail.lastInterimCalculatedAt)}</dd></div></dl>
      {!detail.scoreEligible && <p className="kill-score-warning">참가자 4명 미만의 킬내기는 랭킹 점수가 지급되지 않습니다.</p>}</section>

    {detail.status === "RECRUITING" && <section className="panel kill-actions-panel"><div><h2>참가 모집</h2><p>생성자도 참가하려면 직접 신청해야 합니다.</p></div>
      <div className="kill-actions">{detail.myParticipantId
        ? <button className="secondary-button" disabled={busy} onClick={() => mutate("/participants/me", { method: "DELETE" })}>참가 취소</button>
        : detail.pubgNicknameConfigured ? <button disabled={busy} onClick={() => mutate("/participants/me")}>참가 신청</button>
          : <p className="kill-nickname-guide">커뮤니티 닉네임에서 PUBG 인게임 닉네임을 확인할 수 없습니다. 닉네임 형식을 확인해주세요.</p>}
        {detail.creatorView && <button disabled={busy || !detail.participantCount} onClick={() => mutate("/close-recruitment")}>모집 마감</button>}</div></section>}
    ...
    {canInterim && <section className="panel kill-actions-panel"><div><h2>중간 정산</h2><p>시작부터 지금까지의 전체 기록을 다시 조회한 스냅샷입니다.</p></div>
    ...
    {detail.interimStandings.length > 0 && detail.lastInterimCalculatedAt && <Standings title="중간 정산 결과" standings={detail.interimStandings} />}
    {detail.status === "ENDED" && <section className="panel kill-actions-panel"><div><h2>킬내기가 종료되었습니다.</h2><p>최종 기간 전체를 다시 조회해 결과를 확정합니다.</p></div>
      {detail.creatorView && <button disabled={busy} onClick={() => mutate("/finalize")}>{busy ? "결과 계산 중..." : "결과 발표"}</button>}</section>}
    ...
    {detail.administratorView && !["COMPLETED", "CANCELLED"].includes(detail.status) && <div className="kill-admin-actions">
...
function Roster({ detail }) {
  ...
  if (detail.gameMode === "SOLO" || !detail.teams.length) return <section className="panel kill-roster"><div className="kill-section-heading"><h2>참가자</h2><span>{detail.participantCount}명</span></div>
    <div className="kill-player-grid">{detail.participants.map((p) => <div key={p.participantId}><span><strong>{p.nickname}</strong><small>{p.pubgNickname}</small></span>
  ...
  return <section className="kill-team-grid">{detail.teams.map((team) => <article className="panel kill-team-card" key={team.teamId}><header><h2>{team.name}</h2>
    <strong>{detail.status === "COMPLETED" ? `${team.finalKills}킬` : `${team.interimKills}킬`}</strong></header><div>{detail.participants.filter((p) => p.teamId === team.teamId).map((p) =>
