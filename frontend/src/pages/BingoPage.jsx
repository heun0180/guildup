...
const EMPTY_CATALOG = {items:[],maps:{}};
...
function MissionModal({cell,index,catalog,close,saveCell}) {
  ...
  return <div className="modal-backdrop"><form className="panel bingo-modal" onSubmit={e=>{e.preventDefault();saveCell(index,{...value,targetValue:Number(target)*unitDivisor})}}>
    ...
    {needsWeapon&&<label><span>무기</span><input required value={value.options.weapon||""} onChange={e=>setOption("weapon",e.target.value)} placeholder="M416"/></label>}
    {allowsKillWeapon&&<label><span>무기 필터 (선택)</span><input value={value.options.weapon||""} onChange={e=>setOption("weapon",e.target.value)} placeholder="전체"/></label>}
