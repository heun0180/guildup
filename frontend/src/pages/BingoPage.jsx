...
import { bingoAggregationCooldown, currentBingoScreen, groupManagedBingos, isBingoManagementView } from "../bingoView.js";

const MISSIONS = [
  ["KILLS","킬"],["DAMAGE_DEALT","딜량"],["ASSISTS","어시스트"],["DBNOS","기절"],
  ...
  ["WALL_PENETRATION_KILLS","관통 킬"],["WINS","치킨"],["TOP10","Top 10"],["SURVIVAL_TIME","생존 시간"],
  ["MATCHES_PLAYED","경기 참여"],["HEALS","회복 아이템 사용"],["BOOSTS","부스트 아이템 사용"],
  ["WALK_DISTANCE","도보 이동"],["RIDE_DISTANCE","차량 이동"],["SWIM_DISTANCE","수영"],
  ["PARACHUTE_DISTANCE","낙하산 이동"],["REVIVES","팀원 부활"],["CARRY","다운된 팀원 업기"],
  ["PLAY_WITH_CLAN_MEMBERS","클랜원과 함께 플레이"],["THROWABLE_USED","투척물 사용"],
...
const KM_MISSIONS = new Set(["WALK_DISTANCE","RIDE_DISTANCE","PARACHUTE_DISTANCE"]);
...
const kst = (value) => value ? new Intl.DateTimeFormat("ko-KR", { timeZone:"Asia/Seoul", year:"numeric", month:"2-digit", day:"2-digit", hour:"2-digit", minute:"2-digit" }).format(new Date(value)) : "-";
...
export default function BingoPage() {
  ...
  return <DashboardLayout active="bingos" communityId={communityId}>
    ...
    {cellModal&&(editing?<MissionModal cell={cellModal.cell} index={cellModal.index} close={()=>setCellModal(null)} saveCell={(index,cell)=>{setForm(f=>({...f,cells:f.cells.map((x,i)=>i===index?{...cell,position:index,configured:true}:x)}));setCellModal(null);}}/>:<CellDetail bingo={selected} cell={cellModal} completions={completions} close={()=>setCellModal(null)}/>)}
  </DashboardLayout>;
}
...
function BingoEditor({form,setForm,resize,save,cancel,editCell}){return <form className="bingo-editor" onSubmit={save}><section className="panel bingo-form"><label><span>빙고 이름</span><input disabled={form.locked} value={form.title} maxLength="100" required onChange={e=>setForm({...form,title:e.target.value})}/></label><label className="wide"><span>설명</span><textarea value={form.description} onChange={e=>setForm({...form,description:e.target.value})}/></label><label><span>시작 일시 (KST)</span><input disabled={form.locked} type="datetime-local" required value={form.startsAt} onChange={e=>setForm({...form,startsAt:e.target.value})}/></label><label><span>종료 일시 (KST)</span><input type="datetime-local" required value={form.endsAt} onChange={e=>setForm({...form,endsAt:e.target.value})}/></label><label><span>크기</span><select disabled={form.locked} value={form.boardSize} onChange={e=>resize(Number(e.target.value))}>{[3,4,5].map(n=><option key={n} value={n}>{n} × {n}</option>)}</select></label><label><span>목표 줄 수</span><input disabled={form.locked} type="number" min="1" max={form.boardSize*2+2} value={form.targetLines} onChange={e=>setForm({...form,targetLines:Number(e.target.value)})}/></label><label className="check-field"><input disabled={form.locked} type="checkbox" checked={form.blackoutEnabled} onChange={e=>setForm({...form,blackoutEnabled:e.target.checked})}/>블랙빙고 사용</label><label className="check-field"><input disabled={form.locked} type="checkbox" checked={form.allowLateJoin} onChange={e=>setForm({...form,allowLateJoin:e.target.checked})}/>중간 참가 허용</label></section><BingoGrid size={form.boardSize} cells={form.cells} progress={[]} editable={!form.locked} onCell={(cell,index)=>{if(!form.locked)editCell({cell,index})}}/><div className="bingo-form-actions"><button type="button" className="secondary-button" onClick={cancel}>취소</button><button type="submit">저장</button></div></form>}

function MissionModal({cell,index,close,saveCell}){const [value,setValue]=useState({...cell,options:cell.options||{}});const unitDivisor=KM_MISSIONS.has(value.missionType)?1000:value.missionType==="SURVIVAL_TIME"?60:1;const [target,setTarget]=useState(Number(cell.targetValue||1)/(KM_MISSIONS.has(cell.missionType)?1000:cell.missionType==="SURVIVAL_TIME"?60:1));const needsWeapon=value.missionType==="WEAPON_KILLS",needsCategory=value.missionType==="WEAPON_CATEGORY_KILLS",needsThrowable=["THROWABLE_KILLS","THROWABLE_USED"].includes(value.missionType),needsDistance=value.missionType==="LONG_DISTANCE_KILL",needsClan=value.missionType==="PLAY_WITH_CLAN_MEMBERS";return <div className="modal-backdrop"><form className="panel bingo-modal" onSubmit={e=>{e.preventDefault();saveCell(index,{...value,targetValue:Number(target)*unitDivisor})}}><h2>미션 설정</h2><label><span>미션 종류</span><select value={value.missionType} onChange={e=>{setValue({...value,missionType:e.target.value,options:{}});setTarget(1)}}>{MISSIONS.map(([id,label])=><option value={id} key={id}>{label}</option>)}</select></label><label><span>집계 방식</span><select value={value.aggregationType} onChange={e=>setValue({...value,aggregationType:e.target.value,occurrenceTarget:e.target.value==="MATCH_OCCURRENCES"?1:null})}><option value="EVENT_TOTAL">기간 누적</option><option value="SINGLE_MATCH">한 경기</option><option value="MATCH_OCCURRENCES">조건 달성 경기 수</option></select></label><label><span>목표값{KM_MISSIONS.has(value.missionType)?" (km)":value.missionType==="SURVIVAL_TIME"?" (분)":""}</span><input type="number" min="0" step="0.1" required value={target} onChange={e=>setTarget(Number(e.target.value))}/></label>{value.aggregationType==="MATCH_OCCURRENCES"&&<label><span>달성 경기 수</span><input type="number" min="1" value={value.occurrenceTarget||1} onChange={e=>setValue({...value,occurrenceTarget:Number(e.target.value)})}/></label>}{needsWeapon&&<label><span>무기</span><input required value={value.options.weapon||""} onChange={e=>setValue({...value,options:{weapon:e.target.value}})} placeholder="M416"/></label>}{needsCategory&&<label><span>무기 종류</span><select value={value.options.weaponCategory||"AR"} onChange={e=>setValue({...value,options:{weaponCategory:e.target.value}})}>{CATEGORIES.map(x=><option key={x}>{x}</option>)}</select></label>}{needsThrowable&&<label><span>투척물</span><select value={value.options.throwable||THROWABLES[0]} onChange={e=>setValue({...value,options:{throwable:e.target.value}})}>{THROWABLES.map(x=><option key={x}>{x}</option>)}</select></label>}{needsDistance&&<label><span>거리 (m)</span><input type="number" min="1" value={value.options.distance||200} onChange={e=>setValue({...value,options:{distance:Number(e.target.value)}})}/></label>}{needsClan&&<label><span>본인 제외 클랜원 수</span><input type="number" min="1" value={value.options.clanMemberCount||1} onChange={e=>setValue({...value,options:{clanMemberCount:Number(e.target.value)}})}/></label>}<label><span>표시 문구 (선택)</span><input maxLength="120" value={value.customTitle||""} onChange={e=>setValue({...value,customTitle:e.target.value})}/></label><div className="bingo-modal-actions"><button type="button" className="secondary-button" onClick={close}>취소</button><button type="submit">적용</button></div></form></div>}

function BingoDetail({bingo,viewedBoard,admin,aggregate,remove,edit,back,showCell,viewParticipant}){const board=viewedBoard||bingo.me,progress=board?.progress||[];const [now,setNow]=useState(Date.now());const cooldown=bingoAggregationCooldown(bingo.lastAggregatedAt,now);useEffect(()=>{if(!admin||!cooldown.disabled)return undefined;const timer=window.setInterval(()=>setNow(Date.now()),1000);return()=>window.clearInterval(timer);},[admin,cooldown.disabled,bingo.lastAggregatedAt]);return <>{(back||admin)&&<div className="bingo-detail-actions">{back&&<button className="secondary-button" onClick={back}>목록</button>}{admin&&!["COMPLETED","CANCELLED","SETTLING"].includes(bingo.status)&&<button className="secondary-button" onClick={edit}>수정</button>}{admin&&["ACTIVE","SETTLING"].includes(bingo.status)&&<button onClick={aggregate} disabled={cooldown.disabled} title={cooldown.disabled?"마지막 집계 후 30분이 지나면 다시 집계할 수 있습니다.":undefined}>{cooldown.label}</button>}{admin&&<button className="danger-button" onClick={remove}>{["ACTIVE","SETTLING"].includes(bingo.status)?"취소":"삭제"}</button>}</div>}<section className="panel bingo-overview"><div><span className={`bingo-status status-${bingo.status.toLowerCase()}`}>{bingo.status}</span><h2>{bingo.title}</h2><p>{kst(bingo.startsAt)} ~ {kst(bingo.endsAt)}</p><p>남은 기간 {remaining(bingo.endsAt)}</p></div><dl><div><dt>완료</dt><dd>{bingo.me?.completedCells??0} / {bingo.boardSize*bingo.boardSize}</dd></div><div><dt>현재</dt><dd>{bingo.me?.lineCount??0}줄 빙고</dd></div><div><dt>목표</dt><dd>{bingo.targetLines}줄</dd></div><div><dt>블랙빙고</dt><dd>{!bingo.blackoutEnabled?"사용 안 함":bingo.me?.blackoutCompletedAt?"달성":"진행 중"}</dd></div></dl></section>{bingo.me&&!bingo.me.pubgConnected&&<p className="message">PUBG 계정을 등록해야 빙고 진행 상황을 집계할 수 있습니다.</p>}{viewedBoard&&<p className="bingo-viewing"><strong>{viewedBoard.nickname}</strong>님의 빙고판</p>}<BingoGrid size={bingo.boardSize} cells={bingo.cells} progress={progress} onCell={showCell}/>{bingo.participants.length>0&&<section className="panel bingo-participants"><h2>참가자 달성 현황</h2>{bingo.participants.map(p=><button type="button" key={p.participantId} onClick={()=>viewParticipant(p.participantId)}><strong>{p.nickname}</strong><span>{p.completedCells}/{bingo.boardSize*bingo.boardSize} · {p.lineCount}줄</span></button>)}</section>}</>}
...
function CellDetail({bingo,cell,completions,close}){const p=bingo.me?.progress.find(x=>x.cellId===cell.id);return <div className="modal-backdrop"><section className="panel bingo-modal"><h2>{cell.title}</h2><dl className="bingo-cell-facts"><div><dt>내 진행 상황</dt><dd>{p?.completed?"완료":"진행 중"}</dd></div>{p?.completedAt&&<div><dt>달성</dt><dd>{kst(p.completedAt)}</dd></div>}{p?.evidenceMatchId&&<div><dt>달성에 사용된 경기</dt><dd><code>{p.evidenceMatchId}</code><br/>{kst(p.evidenceEventAt)}</dd></div>}</dl><h3>이 미션을 완료한 클랜원</h3>{completions.length===0?<p className="empty-state">아직 완료한 클랜원이 없습니다.</p>:<ul className="bingo-completions">{completions.map((x,i)=><li key={`${x.nickname}-${i}`}><strong>{x.nickname}</strong><span>{kst(x.completedAt)}</span></li>)}</ul>}<button type="button" className="secondary-button" onClick={close}>닫기</button></section></div>}
